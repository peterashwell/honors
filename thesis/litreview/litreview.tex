...
\section{Distance Measures for Time Series}
	...
	TODO finding outlier light curves in catalogues of periodic variable stars
	...
	\section{Longest Common Subsequence for Time Series}
	...
	\section{Gaussian Processes for regression and smoothing of time series}
...
%	\end{center}
	...
	\section{Summary of Distortion Handling Methods}
...
%	\end{itemize}
	...
	\section{Frequency Domain Approaches}
	\subsection{Introduction}
	...
	\subsection{Phase Invarant Kernels}
	...
	\subsection{Recent Improvements and Distortion-Invariant Forms}
...
%	\end{itemize}
	...
	\begin{figure}[ht!]
		...
		\centering
		\includegraphics[width=100mm]{/Users/peter/honors/thesis/litreview/images/timeseriesmotif.png}
...
%	\begin{figure}[ht!]
%		\label{fig:anytimedistortion}
%		\centering
%		TODO Placeholder for figure of anytime distortion
%		\caption{Figure of the `anytime' distortion in simulated astronomical time series data}
%	\end{figure}
	...
	Strict definitions of motifs in the context of time series lead to algorithms for their extraction and use in classification.
	...
	\begin{figure}[ht!]
		...
		\centering
		\includegraphics[width=140mm]{/Users/peter/honors/thesis/litreview/images/shapelet.eps}
...
%		Motifs are subsequences which are strongly associated with a class, but are probably not very useful since the entire class's lightcurves are a motif in our generic experimental data context.
%		Shapelets are a newer idea which are definitely useful. They can provide robustness to a variety of distortions
		
		\pagebreak %TODO REMOVE
		
		
		 
%	 In a multiclass classification context the motifs of each class are subsequences of training examples of that class which best discriminate between it and other training examples of the same class. Exactly how \emph{discrimination} is defined is the subject of the research discussed in this section. Motifs can be of arbitrary length and a motif can be represented by small variations on the same pattern centered around some representative structure, as polynomials with gaussian variance fitted, and even as symbolic representations of the continuous time series data. An example of a motif for a time series is visually illustrated in figure~\ref{fig:timeseriesmotif}.
%	
%	\subsection{Motifs as strongly distinct clusters}
%	A more explicit definition of motifs and a cluster-like approach to finding motifs of a predefined length $n$ is outlined in \emph{Finding motifs in Time Series} (2002) Keogh, Lin. In addition to a provided length $n$ for the size of the motif, a distance measure for time series, $D$, and a range $R$ need to be given. Exactly what $D$ and $R$ will be depends on the application. In the paper, the distance measure used is the Euclidean distance, a standard metric for comparing time series that was introduced in section~\ref{sec:euclideandistance}. $R$ must be determined by the user and specifies the variation allowed in a motif cluster.
%	
%	Motifs are discovered by finding groupings of datapoints which are at most $R$ away in the distance measure. A simple brute force approach presented in the paper compares all subsequences against each other, each run of the algorithm producing the group with the largest number of co-occurrences of a motif. The elements of each motif so extracted are excluded from any run following to ensure motif distinctness. The authors present a much faster hash-based algorithm that uses a symbolic representation of the time series subsequences and hashing to find candidate motifs in linear time.
%	
%	The issues with this approach to motif extraction are the $n$ and $R$ parameters that need to be defined by the user. Motif length should be determined as the largest possible useful motif for classification purposes. This is not necessarily easy to define either, as it depends on the variability of the time series in question. This algorithm will return many motifs which are subsequences of larger, more significant, possibly more important motifs. Exactly how to define a cut-off point is a serious problem in the application of these ideas to my work.
%	
%	\subsection{Information theoretic definition of motifs}
%	\emph{Discovery of time series motif from multi-dimensional data based on MDL principle}
%	\begin{enumerate}
%		\item PAA transformation of entire time series
%		\item Symbolic representation (SAX) from PAA
%		\item Information theoretic analysis of subsequences of fixed length. Takes into account frequency of subsequence amongst corpora and within the time series itself
%	\end{enumerate}
%	
%	The problems with this approach are that the paper does not explain how to extend motifs to their full length. The symbolic representation reduces the granularity and the discretization is not necessarily a good representation of the original time series. 
%	Uses DTW to somehow find motifs of varying lengths, but explains very poorly how this is done.
%	
%	Clustering of time series subsequences is meaningless (2004), Keogh, Lin: implications for previous and future research
%		- do not use all subsequences as a data points for clustering (duhhh)
%		- use motifs instead: defined as subsequences with high information for a class (duh)
%		- gives a definition of a motif that is functional
%
%	Early Classification of Time Series (2011), Xing, Pei, Yu
%		- Uses hierarchical clustering, 1NN and Euclidean distance with some hacks related to support as a parameter for early classification (support assigned to clusters)
%		- Not really all that interesting
%		- Uses length 0 ... L from start of time series to build clusters. Needs notion of defined start and end to work
%		- Has some good ideas on producing user defined trade offs between earliness of classification and accuracy
%	
% On-line motif detection in time series with SwiftMotif (2009), Fuchs, sGruber, Nitschke, Sick
%		- Defines motifs
%		- Fusion of probabilistic modelling methods and least-squares approximation techniques
%		- Consists of least squares of fitting of polynomials to breakpoints in the time series defined by turning points
%		- Defines a metric for comparing the similarity of two time series defined by segments of polynomials
%
%	
	\section{Astronomical Time Series Classification}
...
%
	...
	\section{Summary and Possible Research Approaches}
...
% TODO add motivation for random forests and the statistical classifier. 
%	\bibliography{refs}
%\end{document}
